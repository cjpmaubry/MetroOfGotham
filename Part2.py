...
#------------------UTILITARY FONCTION-------------------#
    ...
    def MinimumDistance(self, distance, ListeofVertice): 
  
        # initialize minimum distance for next node 
        min = 999999999999999 # Set a big number to initialize
  
        # Loop to browse all the connection for a specific vertex
        for v in range(self.V): 
            if distance[v] < min and ListeofVertice[v] == False: # Define condition to return to minimum distance from this vertex to another
                min = distance[v] 
                min_index = v 
...
#------------Dijkstra Algorithm------------#
    ...
    def Dijkstra(self, start): 
        ...
        distance = [9999999999999] * self.V  # Set a big number to initialize each distance 
        distance[start] = 0 #Initialise the start distance
        ListeofVertice = [False] * self.V # Set all the list of vertice at false status
        ...
        for count in range(self.V): 
            ...
            for v in range(self.V): 
                if self.graph[u][v] > 0 and ListeofVertice[v] == False and distance[u] + self.graph[u][v] <= distance[v] : #Verify the condition
        ...
        DisplayDistance(matrix) #Create an another graph to show the distance
...
def DisplayDistance(matrix):
    ...
    for k in range(0,len(matrix)): 
        #Browse all lines
        id = k
        name=GiveDistance(matrix,k)
        message="distance :"
        plt.text(6, 80-3*k, id,fontsize=9)
        plt.text(8, 80-3*k, message,fontsize=9)
        plt.text(12, 80-3*k, name,fontsize=9)
    plt.axis('off') # Hide the axes
    plt.title("Distance between Gotham and the other town") # Write title


def GiveDistance(matrix,index):
    value=0
